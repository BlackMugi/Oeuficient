...
const Header: React.FC = () => {
  ...
  const incubations: Incubation[] = [
    { lot: 'Lot 3562', type: 'Poule', status: 'J18/18', progress: '100%', color: 'bg-rose-500' },
    { lot: 'Lot 3578', type: 'Poule', status: 'J15/18', progress: '75%', color: 'bg-teal-600' },
    { lot: 'Lot 3592', type: 'Dinde', status: 'J15/28', progress: '50%', color: 'bg-teal-600' },
  ];
    ...
    <div className="min-h-screen bg-slate-50 p-4 md:p-8 font-sans text-slate-900">
      ...
      <header className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 mb-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <h1 className="text-2xl font-bold mt-2">Tableau de bord Responsable</h1>
          <p className="text-slate-500 text-sm">Supervision et gestion d'équipe</p>
        </div>
      ...
      <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        {[
          { label: 'Incubation du jour', value: '2.450', color: 'bg-amber-100' },
      ...
      <main className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <section className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100">
          <div className="flex justify-between items-center mb-6">
            ...
            <span className="text-[10px] bg-slate-100 px-2 py-1 rounded text-slate-500">12 employés</span>
          </div>
          ...
          <button className="mt-6 text-sm text-amber-500 font-bold hover:underline">Voir toute l'équipe →</button>
        </section>

        {/* SECTION INCUBATION */}
        <section className="lg:col-span-2 bg-white p-6 rounded-3xl shadow-sm border border-slate-100">
          <div className="flex justify-between items-center mb-6">
            <h2 className="font-bold text-lg">Incubation en cours <span className="text-[10px] text-slate-400 font-normal ml-1">8 actives</span></h2>
            <button className="bg-amber-400 text-white px-4 py-2 rounded-xl text-xs font-bold hover:bg-amber-500 transition">Nouvelle incubation</button>
          </div>
            ...
            {incubations.map((item, i) => (
              <div key={i} className="group">
                <div className="flex justify-between items-end text-xs mb-2">
                  ...
                  <span className={`${item.color} text-white px-2 py-0.5 rounded text-[10px] font-medium`}>
                    {item.status}
                  </span>
                </div>
                ...
                <p className="text-[10px] text-slate-400 mt-2 flex items-center gap-1">
                   Couvoir C1 (Chariot A et B) • <span className="font-medium text-slate-500">Transfert: Aujourd'hui</span>
                </p>
              </div>
            ))}
          </div>
