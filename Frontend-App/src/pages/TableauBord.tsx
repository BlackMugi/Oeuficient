    ...
    <div className="min-h-screen bg-gray-300 flex justify-center items-center py-10 font-['Barlow_Condensed']">
      <div className="relative w-[1123px] h-[1000px] bg-[#EEECE0] rounded-[10px] shadow-2xl overflow-hidden">
        ...
        <div className="absolute top-0 left-0 w-full h-[120px] bg-[#E7A519] flex items-center px-10 gap-6 text-white z-20">
          <div className="w-16 h-16 bg-white/20 rounded-full flex items-center justify-center text-3xl font-bold">🥚</div>
          <h1 className="text-3xl font-black uppercase">Démarrer une incubation</h1>
        </div>
        ...
        <div className="absolute inset-0 z-10">
          {etape === 1 && <SelectionLotPage selectedId={selectedLot} onSelect={setSelectedLot} />}
        </div>

        {/* Étape 1/4 (Position maquette) */}
        <div className="absolute w-full text-center" style={{ top: '938px' }}>
          <span className="text-[#999] font-medium text-[15px]">Étape {etape} / 4</span>
        </div>

        {/* Rectangle 58 : BOUTON SUIVANT ORANGE */}
        <button 
          onClick={() => etape < 4 && selectedLot && setEtape(prev => prev + 1)}
          style={{ width: '200px', height: '50px', left: '888px', top: '915px', backgroundColor: '#E7A519', borderRadius: '15px' }}
